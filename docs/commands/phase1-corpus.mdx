...
<Note>
No `HF_TOKEN` needed — every source in the default config is public and ungated (see
the Common Voice note above for why that changed). Only set `HF_TOKEN` if you add back
a gated source yourself (`gated: true` in a `sources:` entry).
</Note>
